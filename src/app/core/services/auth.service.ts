    ...
    return this.http.post<AuthModel>(`${this.baseUrl}/auth/login`, { email, password })
      .pipe(
        ...
        catchError(error => {
          ...
          const errorMessage = error?.error?.message || error?.message || 'Error de conexión. Verifica que el backend esté corriendo.';
          return of({ email: '', status: false, message: errorMessage, jwt: '', success: false } as AuthModel);
        })
      );
  }
  logout(): void {
